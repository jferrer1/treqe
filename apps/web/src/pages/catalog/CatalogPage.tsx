...
export function CatalogPage() {
  ...
  useEffect(() => {
    fetch("/mib/v1-catalogo.html").then(r => r.text()).then(raw => {
      ...
      s = s.replace("</style>", ".item-card{display:flex;flex-direction:column;height:100%}.item-card__info{flex:1;background:var(--bg)}.search-expand{top:59%;right:100px!important}</style>");
      setHtml(s + b);
    });
  }, []);
