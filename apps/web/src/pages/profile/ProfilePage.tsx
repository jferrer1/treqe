...
function renderProduct(p: Product, asLiked = false): string {
  return `<div class="my-item" onclick="window.location.hash='#/articulo/${p.id}'">
...
export function ProfilePage() {
  ...
  useEffect(() => {
    fetch(`${BASE}mib/v4-perfil.html`).then(r => r.text()).then(raw => {
      ...
      // Inject Salir button next to dark toggle — wired via click handler below
  ...
  if (!hasToken) {
    ...
    // Remove gear icon and salir button (no session)
    styledHtml = styledHtml.replace(/<a[^>]*>[^<]*<i class="fas fa-cog"><\/i><\/a>/g, '');
    styledHtml = styledHtml.replace(/<button class="salir-btn"[^>]*>[^<]*<\/button>/g, '');
